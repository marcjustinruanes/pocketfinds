...
@section('page-sub', 'Communicate with admins, logistics, and your sellers/buyers')
...
<style>
  ...
  .chat-msg-wrap .chat-bubble-out ~ .chat-doc-bubble,.chat-bubble-out + .chat-doc-bubble{background:rgba(255,255,255,.2);border-color:transparent;color:#fff}
  ...
  <div class="chat-list">
    <div class="chat-list-head">
      <div style="position:relative">
        ...
        <input type="text" data-table-search style="width:100%;border:1px solid var(--border);border-radius:9px;padding:8px 12px 8px 32px;font-size:13px;font-family:var(--font-body)" placeholder="Search…">
      </div>
    </div>

    @foreach(['admins' => 'Admin', 'logistics' => 'Logistics', 'contacts' => 'Buyers & Sellers'] as $group => $label)
      @if(${$group}->isNotEmpty())
      ...
      @foreach(${$group} as $u)
      <a href="{{ route('rider.messages.thread', $u->id) }}" class="chat-list-item {{ isset($activeUser) && $activeUser?->id == $u->id ? 'active' : '' }} {{ $u->unread_count > 0 ? 'chat-unread' : '' }}">
        ...
        <div class="cli-body">
          ...
          <div class="cli-role">{{ $group === 'admins' ? 'Administrator' : ($group === 'logistics' ? 'Logistics' : ucfirst($u->account_type)) }}</div>
  ...
  <div class="chat-main">
    ...
    <div class="chat-head">
      ...
      <div class="chat-head-info">
        ...
        <div class="chat-head-sub">{{ $activeUser->is_admin ? 'Administrator' : ($activeUser->is_logistics ? 'Logistics' : ucfirst($activeUser->account_type)) }} · {{ $activeUser->email }}</div>
      </div>
    </div>
    <div class="chat-body" id="chatBody">
      @forelse($messages as $msg)
        @include('rider.partials.message-bubble', ['msg' => $msg, 'myId' => $myId])
      @empty
  ...
  <div class="modal" style="max-width:520px">
    ...
    <form id="reportForm" enctype="multipart/form-data">
      <div class="modal-body">
        ...
        <div class="form-row"><label for="reportReason">Why are you reporting this?</label>
...
const MY_ID      = {{ $myId }};
const RECEIVER   = {{ $activeUser->id }};
const SEND_URL   = '{{ route('rider.messages.send') }}';
const POLL_URL   = '{{ route('rider.messages.poll') }}';
const REPORT_URL = '{{ route('rider.messages.report') }}';
...
document.getElementById('reportForm').addEventListener('submit', async (e) => {
  ...
  try {
    const res = await fetch(REPORT_URL, { method: 'POST', body: new FormData(e.target), headers: { Accept: 'application/json', 'X-CSRF-TOKEN': CSRF } });
    const data = await res.json();
    if (!res.ok || !data.ok) throw new Error(data.message || 'Report could not be sent.');
    closeReportModal();
    alert('Report sent to admin.');
  } catch (err) {
    const box = document.getElementById('reportError'); box.textContent = err.message; box.style.display = 'block';
...
function actionButtonsHtml() {
  return `<div class="chat-msg-actions">
    ...
    <button type="button" class="chat-msg-action" title="Report" onclick="openReportModal(this)"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 21V4m0 1h10l-2 3 2 3H5"/></svg></button>
  </div>`;
}

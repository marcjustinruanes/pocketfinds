...
<div class="chat-shell">
  ...
  <div class="chat-main">
    ...
    @if($seller)
      ...
      @if($product)
      @php $productJson = json_encode(['id'=>$product->id,'name'=>$product->name,'price'=>$product->price,'img'=>$product->image ? Storage::url($product->image) : null,'url'=>route('buyer.product',$product->id)]); @endphp
      <div class="chat-attachment" id="chatAttachment">
        <div class="chat-attach-inner">
          <div class="chat-attach-img">
            @if($product->image)<img src="{{ Storage::url($product->image) }}" style="width:100%;height:100%;object-fit:cover;border-radius:6px">
            @else @include('buyer.partials.icon', ['name' => 'bag', 'size' => 20]) @endif
          </div>
          <div class="chat-attach-info">
            <div class="chat-attach-label">Product</div>
            <div class="chat-attach-name">{{ $product->name }}</div>
            <div class="chat-attach-price">₱{{ number_format($product->price) }}</div>
          </div>

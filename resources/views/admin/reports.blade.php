...
<div class="kpi-grid" style="margin-bottom:18px">
  ...
  <div class="kpi tone-info"><div class="label">Buyers</div><div class="value">{{ number_format($buyerCount) }}</div></div>
  <div class="kpi"><div class="label">Sellers</div><div class="value">{{ number_format($sellerCount) }}</div></div>
  <div class="kpi tone-warning"><div class="label">Riders</div><div class="value">{{ number_format($riderCount) }}</div></div>
</div>

<div class="kpi-grid" style="margin-bottom:24px">
  <div class="kpi tone-success"><div class="label">Total Commission</div><div class="value">₱{{ number_format($totalCommission, 2) }}</div><div class="delta up">Platform earnings</div></div>
  <div class="kpi"><div class="label">Commission Records</div><div class="value">{{ number_format($commissionCount) }}</div></div>
  <div class="kpi tone-success"><div class="label">Approved Users</div><div class="value">{{ number_format($approved) }}</div><div class="delta up">Active accounts</div></div>
  <div class="kpi tone-warning"><div class="label">Pending</div><div class="value">{{ number_format($pending) }}</div><div class="delta {{ $pending > 0 ? 'down' : '' }}">Awaiting review</div></div>
</div>
  ...
  <div class="card">
    <div class="card-head">
      ...
      <div style="display:flex;gap:6px">
        <a href="{{ route('admin.reports.export.sales') }}" class="btn btn-outline btn-sm"><x-admin-icon name="download" /> CSV</a>
        <a href="{{ route('admin.reports.export.sales-pdf') }}" class="btn btn-primary btn-sm"><x-admin-icon name="download" /> PDF</a>
      </div>
    </div>
  ...
  <div class="stack">
    ...
    <div class="card">
      <div class="card-head">
        ...
        <a href="{{ route('admin.reports.export.commission') }}" class="btn btn-primary btn-sm"><x-admin-icon name="download" /> Export CSV</a>
      </div>
      <div class="card-pad" style="display:flex;flex-direction:column;gap:12px">
        ...
        <a href="{{ route('admin.reports.export.sales') }}" class="btn btn-outline btn-block"><x-admin-icon name="file" /> Download Sales Summary (CSV)</a>
        <a href="{{ route('admin.reports.export.sales-pdf') }}" class="btn btn-outline btn-block"><x-admin-icon name="file" /> Download Sales Summary (PDF)</a>
      </div>
    </div>
